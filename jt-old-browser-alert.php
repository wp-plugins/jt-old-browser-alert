 ...
 * Description: 하위 브라우저로 접속했을시 사이트 상단에 최신 브라우저로 업데이트를 권하는 알림창을 띄웁니다.
 * Version: 1.0.2
 ...
 * Add Action
 ---------------------------------------------------------------------------*/
add_action('wp_head', 'jt_old_browser_style', 1, 1);
add_action('wp_footer', 'jt_old_browser_script', 10, 1);
add_action('wp_footer', 'jt_old_browser_alert', 1, 1);

/*----------------------------------------------------------------------------
 * Script & Stylesheet
 ---------------------------------------------------------------------------*/
function jt_old_browser_style(){
    wp_enqueue_style('jt-old-browser-alert-style', Plugins_url('/css/jt-style.css', __FILE__), false, '1.0.2', 'all');
}
